...
# Standard libraries for file I/O, time, and metrics
import json
...
# Importing CLIP model for contrastive training
import clip
import numpy as np
import pandas as pd
# PyTorch modules for model, optimizer, and tensor operations
import torch
...
from data_utils import base_path, squarepad_transform, targetpad_transform, FashionIQDataset, CIRRDataset
...
# Evaluate retrieval performance on FashionIQ validation split
from validate import compute_fiq_val_metrics,compute_cirr_val_metrics

# Main training loop for fine-tuning CLIP on FashionIQ
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(base_path / f"models/clip_finetuned_on_fiq_{clip_model_name}_{training_start}")
    ...
    # clip_model refers to the main CLIP model object, which includes the vision and text encoders needed for tasks such as image and text processing.
    # clip_preprocess is a preprocessing function used to convert input data (images and text) into a format suitable for the CLIP model.
    ...
    if transform == "clip":
        ...
        #preprocessing pipline/strategy has been developed/assigned
    ...
    for idx, dress_type in enumerate(val_dress_types):
        # these three lines of code generate an array indexed with reference image name, target image name and relevant captions
        idx_to_dress_mapping[idx] = dress_type
    # Load FashionIQ validation dataset
    ...
    # Load FashionIQ validation dataset
        ...
        # Till here as wee need to train both the text and image encoder

    # Define the train datasets and the combining function
    # Load FashionIQ training dataset with selected transformation
    ...
    # Define optimizer to update mapping layers or full model
...
# Start with the training loop
    ...
    # Epoch-wise training loop for model fine-tuning
...
# validation stage is performed over here
...
if __name__ == '__main__':
    ...
    parser.add_argument("--api-key", type=str, help="api for Comet logging", default="FcbENViqrqi4ccOsLSgnXXTGy")
    parser.add_argument("--workspace", type=str, help="workspace of Comet logging", default="zafrankhan1830-gm-gist-ac-kr")
    parser.add_argument("--experiment-name", type=str, help="name of the experiment on Comet", default="fashioniq-clip-fine-tuning")
